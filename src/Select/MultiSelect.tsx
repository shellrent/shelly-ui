...
const MultiSelect = forwardRef<HTMLInputElement, MultiSelectProps>(({ displayFn, defaultOption, showEmptyOption, validators, name, onChange, onValueChange, placeholder, inputSize, error, ...props }, ref) => {
	...
	return (
		<Listbox ref={ref} value={selectedValues === undefined ? '' : selectedValues} onChange={onSelectChange} name={name} multiple>
				...
				<Listbox.Button className={classNames}>
					<span className="overflow-hidden h-full flex items-center">
						...
						{selectedOptions?.map((opt, key) => (
							<span className={`mx-1 my-auto ${opt.disabled && 'text-base-content/40 '}`} key={key}>
								{displayFn ? displayFn(opt) : opt?.title}
							</span>
						))}
					</span>
				</Listbox.Button>
